'use strict'
var conta = document.querySelector('.tea__list');
var btns = conta.querySelectorAll('.tea__list-item');
var teaContent =  document.querySelector('.tea__content');
var teaItem = teaContent.querySelectorAll('.tea__content-item');

for (var i = 0; i < btns.length; i++) {
  btns[i].addEventListener('click', function(evt) {
    evt.preventDefault();
    var current = document.getElementsByClassName('tea__list-item--active');
    current[0].className = current[0].className.replace( 'tea__list-item--active', '');
    this.className += ' tea__list-item--active';
  });
}
